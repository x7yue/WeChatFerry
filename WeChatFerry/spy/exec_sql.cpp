...
int GetLocalIdandDbidx(uint64_t id, uint64_t* localId, uint32_t* dbIdx, uint64_t* type)
{
    ...
    *dbIdx = 0;
    for (int i = dbIndex - 1; i >= 0; i--) { // 从后往前遍历
        ...
        if (dbAddr) {
            ...
            {
                DbField_t field = row[0];
                if ((field.column.compare("type") != 0) && (field.type != 1)) {
                    continue;
                }
                if (type)
                {
                    auto data = field.content;
                    data.push_back('\0');

                    LOG_DEBUG("field type: {} {}", (const char*)(data.data()), data.size());
                    *type = strtoull((const char*)(data.data()), NULL, 10);
                }

            }

            {
                DbField_t field = row[1];
                ...
                LOG_DEBUG("field localId: {} {}", (const char*)(data.data()), data.size());
                *localId = strtoull((const char*)(data.data()), NULL, 10);
